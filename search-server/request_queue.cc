...
std::vector<Document> RequestQueue::AddFindRequest(const std::string& raw_query) {
    std::vector<Document> result = search_server_.FindTopDocuments(raw_query, DocumentStatus::ACTUAL);
...
void RequestQueue::ProcessQueue(const std::string& raw_query, const std::vector<Document>& result, bool is_empty, DocumentStatus status) {
